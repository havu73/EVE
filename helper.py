...
def print_command(code_file, namespace):
	'''
	namespace is a NameSpace object, as part of argparser. We can convert it to a dictionary using vars(namespace). this functions outputs the whole command line argument as a string
	'''
	result = 'python {}'.format(code_file)
	namespace = vars(namespace)
	vars_list =  ["--{0} {1}".format(x, namespace[x]) for x in namespace]
	result += ' '.join(vars_list)
	return result

def save_gzip_dataframe_with_comment(df, comment, output_fn, header = True, index = False, sep = '\t'):
	'''
	this function assumes that output_fn does NOT have .gz tail
	'''
	remove_file('{}.gz'.format(output_fn)) # remove it first because we will write it
	outF = open(output_fn, 'a')
	outF.write('#' + comment + '\n')
	df.to_csv(outF, header = header, index = index, sep = sep)
	outF.close()
	subprocess.run('gzip -f {}'.format(output_fn), shell = True)
	print ('Done saving file {}.gz'.format(output_fn))
	return 
